...
from .models import (ProcedureType, Session, WaterAdministration, Weighing, WaterType,
                     WaterRestriction)
from subjects.models import Subject, Project, Subproject, Winstor_session
...
SESSION_FIELDS = ('subject', 'users', 'location', 'procedures', 'lab', 'project', 'subproject', 'winstor_session', 'type',
...
class SessionListSerializer(BaseActionSerializer):
    ...
    subproject = serializers.SlugRelatedField(read_only=False,
                                           slug_field='name',
                                           queryset=Subproject.objects.all())

    winstor_session = serializers.SlugRelatedField(read_only=False,
                                           slug_field='name',
                                           queryset=Winstor_session.objects.all())
    @staticmethod
    def setup_eager_loading(queryset):
        """ Perform necessary eager loading of data to avoid horrible performance."""
        queryset = queryset.select_related('subject', 'lab', 'project', 'subproject', 'winstor_session')
        return queryset.order_by('-start_time')

    class Meta:
        model = Session
        fields = ('subject', 'start_time', 'number', 'lab', 'project', 'subproject', 'winstor_session', 'url', 'task_protocol', 'data_dataset_session_related', 'dset_types')
...
class SessionDetailSerializer(BaseActionSerializer):
    ...
    subproject = serializers.SlugRelatedField(read_only=False, slug_field='name', many=False,
                                           queryset=Subproject.objects.all(), required=False)
    winstor_session = serializers.SlugRelatedField(read_only=False, slug_field='name', many=False,
                                           queryset=Winstor_session.objects.all(), required=False)
